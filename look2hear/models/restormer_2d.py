...
class Recurrent(nn.Module):
    def __init__(self, dim, num_blocks, heads, ffn_expansion_factor, _iter=6):
        super().__init__()
        self.unet = DPT(dim,
                        num_blocks, 
                        heads,
...
class restormer_2d(nn.Module):
    def __init__(self, 
        ...
        super(restormer_2d, self).__init__()
...
if __name__ == '__main__':
    ...
    model = restormer_2d().to(device)
    input_signal = torch.randn(4,1,32000).to(device)  # 输入图像的形状 (C, H, W)
    v = torch.randn(4,512,50).to(device)
