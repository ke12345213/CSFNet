...
class global_channel_attention(nn.Module):
    ...
    def forward(self, x, v, l):
        ...
        x_out = self.psa_a(x_out)
        v_out = self.psa_v(v_out)
        l_out = self.psa_l(l_out)
...
class tfgridnet_v3(nn.Module):
    def __init__(self, 
        ...
        super(tfgridnet_v3, self).__init__()
        ...
        # state_dict = torch.load('/root/dataset/LRS2/frame_112/best_model_epoch_95_0.94.pt', map_location='cuda')  # 或 'cuda'
        ...
        self.inter = nn.ModuleList([
            global_channel_attention(dim * 4, dim * 4, dim * 4) for _ in range(1)
        ])
    ...
    def forward(self, x, v, face=None):  # 这里应该是face
        ...
        for i in self.inter:
            inp_enc_level1_1, v00, lip_emb0 = i(inp_enc_level1, v00, lip_emb0)
            inp_enc_level1_2, v01, lip_emb1 = i(inp_enc_level1, v01, lip_emb1)

        fusion0 = torch.cat([inp_enc_level1_1, v00, lip_emb0], dim=1)  # [B, 512, T, F] 192+192+192  # 576
        fusion1 = torch.cat([inp_enc_level1_2, v01, lip_emb1], dim=1)  # [B, 512, T, F] 192+192+192  # 576
...
if __name__ == '__main__':
    ...
    model = tfgridnet_v3().to(device)
    # print(model)
    input_signal = torch.randn(1,32000).to(device)  # 输入图像的形状 (C, H, W)
    v = torch.randn(1,2,512,50).to(device)
