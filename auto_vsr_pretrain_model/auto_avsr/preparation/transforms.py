...
class TextTransform:  # 这是一个用于 SentencePiece 分词 + token ID 映射的类
    """Mapping Dictionary Class for SentencePiece tokenization."""
     # tokenize(text)：把一句文本转成 token ID 列表（tensor）
     # post_process(token_ids)：把 token ID 序列还原成文本

    def __init__(
        self,
        sp_model_path=SP_MODEL_PATH,  # SentencePiece 模型文件（.model）
        dict_path=DICT_PATH,  # 单位（token）到 ID 的映射文件（如 dict.txt）
    ):

        # Load SentencePiece model 载入 SentencePiece 模型，用于将文本切分为 subword。
        ...
        self.hashmap = {unit.split()[0]: unit.split()[-1] for unit in units}  # hashmap 是 token -> id 的字典。
        # 0 will be used for "blank" in CTC
        self.token_list = ["<blank>"] + list(self.hashmap.keys()) + ["<eos>"]  # 这是用来在反解时用 ID 映射回 token 的 token 列表（id -> token）
        self.ignore_id = -1

    def tokenize(self, text):  # tokenize 方法（文本 -> token ID）
        tokens = self.spm.EncodeAsPieces(text)  # spm.EncodeAsPieces(text) 把 "hello world" 切成 ["▁hello", "▁world"]
        token_ids = [self.hashmap.get(token, self.hashmap["<unk>"]) for token in tokens] # 每个 subword token 转成 id：["▁hello", "▁world"] -> [42, 85]
        return torch.tensor(list(map(int, token_ids)))  # 返回的是 torch.tensor([42, 85])


    def post_process(self, token_ids):  # post_process 方法（token ID -> 文本）
        token_ids = token_ids[token_ids != -1]
        text = self._ids_to_str(token_ids, self.token_list)
        text = text.replace("\u2581", " ").strip()
        return text  # 用于把 [42, 85] 转为 "hello world"，其中 ▁ 被替换为空格。
